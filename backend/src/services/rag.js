...
export async function retrieveExamplesForWord(wordId, k = 3) {
  const word = await db.prepare('SELECT id, lemma, level, meaning_zh FROM words WHERE id = ?').get(wordId);
...
export async function corpusStats() {
  const chunks = (await db.prepare('SELECT COUNT(*) AS c FROM corpus_chunks').get()).c;
  const linked = (await db.prepare('SELECT COUNT(*) AS c FROM example_cache').get()).c;
