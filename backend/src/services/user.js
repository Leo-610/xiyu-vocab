...
export function updateUserProfile(db, userId, { nickname }) {
  ...
  db.prepare('UPDATE users SET nickname = ? WHERE id = ?').run(name, userId)
}

export function saveUserAvatar(db, userId, buffer, mimeType) {
  if (!buffer?.length) {
