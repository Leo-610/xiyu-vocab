...
export function getExamPack(db, packId, userId, totalCount = 10) {
  ...
  return {
    ...
    words: [
      ...
      ...mapWordRows(db, newRows, 'exam'),
      ...mapWordRows(db, extraRows, 'exam'),
    ],
  }
}

export function getExamSummaries(db, userId) {
  return Object.values(EXAM_PACKS).map((pack) => {
    const pattern = tagLikePattern(pack.tag)
    const total = db.prepare('SELECT COUNT(*) AS c FROM words WHERE tags LIKE ?').get(pattern).c
