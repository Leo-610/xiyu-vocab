...
async function findCachedReview(kind, wordId, hash) {
...
export async function explainMistake({ wordId, wrongChoice, selectedText }) {
  const { word, examples } = await retrieveExamplesForWord(wordId, 3);
  if (!word) {
  ...
  const cached = await findCachedReview('mistake_explain', wordId, hash);
  if (cached) {
    let output;
...
export async function listAiReviews({ status = 'pending', limit = 50 } = {}) {
...
export async function reviewAiItem(id, { status, reviewer }) {
  if (!['approved', 'rejected'].includes(status)) {
    const err = new Error('status 须为 approved 或 rejected');
    err.code = 'INVALID_STATUS';
    throw err;
  }
  const row = await db.prepare('SELECT * FROM ai_reviews WHERE id = ?').get(id);
  if (!row) {
  ...
  return (await listAiReviews({ status: 'all', limit: 1 })).find((x) => x.id === id) ||
  { id, status };
}

export async function llmStatus() {
  const cfg = getLlmConfig();
  const pending = (await db.prepare(`SELECT COUNT(*) AS c FROM ai_reviews WHERE status = 'pending'`).get()).c;
  const approved = (await db.prepare(`SELECT COUNT(*) AS c FROM ai_reviews WHERE status = 'approved'`).get()).c;
  return {
