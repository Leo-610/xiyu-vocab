...
export async function updateUserSettings(db, userId, partial) {
  const row = await db.prepare('SELECT settings_json FROM users WHERE id = ?').get(userId);
...
export async function logStudyEvent(db, userId, {
  wordId,
  eventType = 'answer',
  isCorrect,
  studyMode = null,
...
export async function getStudySummary(db, userId, days = 30) {
