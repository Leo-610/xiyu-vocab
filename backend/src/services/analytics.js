...
export async function getPilotReport(db) {
  const today = todayStr();
  const weekAgo = daysAgo(7);
  const monthAgo = daysAgo(30);

  const usersTotal = (await db.prepare('SELECT COUNT(*) AS c FROM users').get()).c;
  const wechatUsers = (await db.prepare(`
  ...
  const active7d = (await db.prepare(`
    ...
    WHERE session_date >= ?
  `).get(weekAgo)).c;
  const active30d = (await db.prepare(`
    SELECT COUNT(DISTINCT user_id) AS c FROM daily_sessions
    WHERE session_date >= ?
  `).get(monthAgo)).c;

  const sessionAgg = await db.prepare(`
    SELECT
      COUNT(*) AS sessions,
  ...
  const checkinsTotal = (await db.prepare('SELECT COUNT(*) AS c FROM checkin_log').get()).c;
  const avgStreak = (await db.prepare(`
    SELECT ROUND(AVG(streak_days), 1) AS v FROM users WHERE streak_days > 0
  `).get()).v || 0;

  const wordsLearned = (await db.prepare(`
    SELECT COUNT(*) AS c FROM user_word_progress
    WHERE status IN ('learning', 'mastered')
  `).get()).c;
  const mistakesOpen = (await db.prepare(`
    SELECT COUNT(*) AS c FROM mistake_book WHERE resolved = 0
  `).get()).c;
  const dueReview = (await db.prepare(`
    SELECT COUNT(*) AS c FROM user_word_progress
    WHERE status = 'learning' AND date(next_review) <= date(?)
  `).get(today)).c;

  const levelLearned = await db.prepare(`
  ...
  const topMistakes = await db.prepare(`
