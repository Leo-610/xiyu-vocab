...
function recordEvent(key) {
  db.prepare('INSERT INTO rate_limit_events (key) VALUES (?)').run(key)
}

export function checkRateLimit(scope, identifier) {
  const config = LIMITS[scope]
  if (!config) return { allowed: true }

  const key = buildKey(scope, identifier)
  const recent = countRecent(key, config.windowMs)
